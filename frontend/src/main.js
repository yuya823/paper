  ...
  _buildLoginScreen() {
    ...
    s.innerHTML = `
      <div class="upload-hero">
        ...
        <p>英語の学術論文PDFを日本語に翻訳し<br>左右に並べて対訳で読めます。</p>
      </div>
      <div class="auth-card">
        <div class="auth-tabs">
        ...
        <form class="auth-form" id="authForm">
          <input type="email" id="authEmail" placeholder="メールアドレス" required
            style="width:100%;padding:10px 14px;background:var(--c-bg);border:1px solid var(--c-border);border-radius:var(--radius-sm);color:var(--c-text);font-size:0.9rem;margin-bottom:10px;" />
          <input type="password" id="authPassword" placeholder="パスワード（6文字以上）" required minlength="6"
            style="width:100%;padding:10px 14px;background:var(--c-bg);border:1px solid var(--c-border);border-radius:var(--radius-sm);color:var(--c-text);font-size:0.9rem;margin-bottom:14px;" />
          <button type="submit" class="btn btn--primary" style="width:100%;padding:12px;font-size:0.95rem;" id="authSubmit">ログイン</button>
        </form>
        <div style="text-align:center;margin:16px 0 12px;color:var(--c-text-muted);font-size:0.8rem;">または</div>
        <button class="btn" id="btnGoogle" style="width:100%;padding:12px;background:var(--c-bg);border:1px solid var(--c-border);border-radius:var(--radius-sm);font-size:0.9rem;justify-content:center;">
        ...
        <div id="authError" style="color:var(--c-error);font-size:0.8rem;text-align:center;margin-top:12px;display:none;"></div>
      </div>
    `;
    ...
    setTimeout(() => {
      ...
      tabs.forEach(tab => {
        tab.addEventListener('click', () => {
          ...
          s.querySelector('#authSubmit').textContent = isLogin ? 'ログイン' : '新規登録';
        });
      });
      ...
      s.querySelector('#authForm').addEventListener('submit', async (e) => {
        ...
        const errEl = s.querySelector('#authError');
        ...
        try {
          if (isLogin) {
            ...
            errEl.textContent = '確認メールを送信しました。メールを確認してください。';
            errEl.style.display = 'block';
            errEl.style.color = 'var(--c-success)';
            return;
          }
        } catch (err) {
          errEl.textContent = err.message;
          errEl.style.display = 'block';
          errEl.style.color = 'var(--c-error)';
        }
      });

      s.querySelector('#btnGoogle').addEventListener('click', async () => {
        try { await signInWithGoogle(); } catch (err) {
          s.querySelector('#authError').textContent = err.message;
          s.querySelector('#authError').style.display = 'block';
        }
      });
    });
    return s;
